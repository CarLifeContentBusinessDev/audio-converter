...
const TMP_DIR = 'C:\\Users\\250901\\AppData\\Local\\Temp\\gongu-download';
...
async function main() {
  ...
  if (toDownload.length > 0) {
    ...
    for (const { wrtSn, filename } of toDownload) {
      process.stdout.write(`  [${done + failed + 1}/${toDownload.length}] ${filename} ... `);
      try {
    ...
    if (failedItems.length > 0) {
      ...
      for (const { wrtSn, filename } of failedItems) {
        ...
        try {
          ...
          logError(`재시도 실패 wrtSn:${wrtSn} file:${filename} err:${err.message}`);
        }
      }
    }
  }
  ...
  // 6. 최종 결과
  const finalCount = fs.readdirSync(SAVE_DIR).filter(f => f.endsWith('.mp3')).length;
  console.log('\n=== 동기화 완료 ===');
